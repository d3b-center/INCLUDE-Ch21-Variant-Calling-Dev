...
steps:
  gatk_intervallist_to_bed:
    run: ../kf-alignment-workflow/tools/gatk_intervallist_to_bed.cwl
    hints:
    ...
    out: [output]
  bedtools_intersect:
    run: ../kf-alignment-workflow/tools/bedtools_intersect.cwl
    hints:
    - class: "sbg:AWSInstanceType"
      value: c6i.2xlarge
    in:
  ...
  samtools_cram_to_bam:
    run: ../kf-alignment-workflow/tools/samtools_cram_to_bam.cwl
    in:
  ...
  verifybamid_checkcontam_conditional:
    run: ../kf-alignment-workflow/tools/verifybamid_contamination_conditional.cwl
    hints:
    - class: "sbg:AWSInstanceType"
      value: c6i.2xlarge
    in:
  ...
  sentieon_haplotyper:
    ...
    run: ../kf-alignment-workflow/tools/sentieon_haplotyper.cwl
    in:
  ...
  picard_intervallisttools:
    ...
    run: ../kf-alignment-workflow/tools/picard_intervallisttools.cwl
    in:
      interval_list: re_calling_interval_list
      use_sentieon: use_sentieon
    out: [output]
  ...
  gatk_haplotypecaller:
    ...
    hints:
      - class: sbg:AWSInstanceType
        value: c6i.4xlarge
    run: ../kf-alignment-workflow/tools/gatk_haplotypecaller.cwl
    in:
  ...
  picard_mergevcfs_python_renamesample:
    run: ../kf-alignment-workflow/tools/picard_mergevcfs_python_renamesample.cwl
    hints:
    - class: "sbg:AWSInstanceType"
      value: c6i.2xlarge
    in:
  ...
  picard_collectgvcfcallingmetrics:
    run: ../kf-alignment-workflow/tools/picard_collectgvcfcallingmetrics.cwl
    in:
      dbsnp_vcf:
        source: [dbsnp_vcf, dbsnp_idx]
        valueFrom: |
  ...
  picard_mergevcfs:
    ...
    out: [output]
  sentieon_gvcftyper:
    run: ../kf-alignment-workflow/tools/sentieon_gvcftyper.cwl
    in:
      sentieon_license: sentieon_license
      indexed_reference_fasta: reference_fasta
